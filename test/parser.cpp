...
std::unordered_map<std::string, Expression*> expressions = {
    ...
    std::make_pair("x-2+x", new Addition(new Variable("x"), new Addition(new Constant(-2), new Variable("x")))),
    std::make_pair("x-2", new Addition(new Variable("x"), new Constant(-2))),
    std::make_pair("sin(x-2)", new Sin(new Addition(new Variable("x"), new Constant(-2))))
};
